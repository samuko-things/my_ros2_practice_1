...
public:
  BatteryClientNode() : Node("battery_client")
  {
    battery_timer_ = this->create_wall_timer(
        ...
        std::bind(&BatteryClientNode::simulate_battery, this));
    RCLCPP_INFO(this->get_logger(), "Battery Client node has been started.");
  }
...
private:
  void simulate_battery()
  {
    ...
    threads_.push_back(std::thread(std::bind(&BatteryClientNode::call_led_state_service, this, 2, battery_full)));
  }

  void call_led_state_service(int req_led_number, bool req_turn_on)
  {
    auto client = this->create_client<my_robot_interfaces::srv::SetLed>("set_led");
    while (!client->wait_for_service(std::chrono::seconds(1)))
    {
      RCLCPP_WARN(this->get_logger(), "Waiting for the led_state_server to be up...");
    }

    auto req = std::make_shared<my_robot_interfaces::srv::SetLed::Request>();
    req->led_number = req_led_number;
    req->turn_on = req_turn_on;
    ...
    try
    {
      ...
      if (req_turn_on){
        RCLCPP_INFO(this->get_logger(), "led number %d is on", (int)req->led_number);
      } else {
        RCLCPP_INFO(this->get_logger(), "led number %d is off", (int)req->led_number);
      }
      
    }
    catch (const std::exception &e)
    {
      RCLCPP_ERROR(this->get_logger(), "Error while calling led_state_service");
    }
  }
